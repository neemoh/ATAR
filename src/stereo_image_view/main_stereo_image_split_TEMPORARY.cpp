...
int main(int argc, char **argv)
{
    ...
    pub[0] = it.advertise("/overlay/left/image", 1);;
    pub[1] = it.advertise("/overlay/right/image", 1);;
    ...
    while (ros::ok())
    {
        ...
        if(new_left_image && new_right_image || new_stereo_image) {
            ...
            for (int j = 0; j <2 ; ++j) {
                ...
                cv::Mat out;
                cv::cvtColor(image[j], out, CV_BGR2BGRA);

                pub[j].publish(
                    cv_bridge::CvImage(std_msgs::Header(),
                                       "bgra8", out).toImageMsg());
            }
        }
