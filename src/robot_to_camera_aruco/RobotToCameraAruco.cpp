...
Eigen::Vector3d	RobotToCameraAruco::FindAxis(const Eigen::MatrixXd &axis_points){
		...
		cv::Matx33d &rotm, cv::Vec3d &br_tvec)
{
	...
	for (unsigned int i=0; i<n_points; i++)	points.row(i) = axis_points[i];
	...
	for (unsigned int iter=0; iter<n_points; iter++)
		projection_vectors.row(iter) = dist(iter) * plane_normal;

	Eigen::MatrixXd points_projected = points - projection_vectors;

	// ---------------  fit the x axis
	Eigen::Vector3d x_axis = FindAxis(points_projected.topRows(n_points_per_axis));


	//  --------------- fit the y axis
	Eigen::Vector3d y_axis = FindAxis(points_projected.bottomRows(n_points_per_axis));
